...
public class FadeObjectBlockingObject : MonoBehaviour
{
    ...
    private IEnumerator CheckForObjects()
    {
        ...
        while (true)
        {
            int hits = Physics.RaycastNonAlloc(mainCamera.transform.position, (Player.transform.position - mainCamera.transform.position)
                .normalized, Hits, Vector3.Distance(mainCamera.transform.position, Player.transform.position), LayerMask);
            if (hits > 0)
            {
